...
void defaultHandleKeys(unsigned int key, int mousex, int mousey)
{
	...
	if (key)
	{
		...
		{
			case 27:
				EsgiGLApplication::g_ThisInstance->Exit();
				break;
			default:
				break;
		}
...
void EsgiGLApplication::HandleKeyboard(unsigned char key, int mx, int my)
{
	if (m_KeyFunc == NULL) {
...
void EsgiGLApplication::HandleKeyboard(int key, int mx, int my)
{
	if (m_KeySpecialFunc == NULL) {
		defaultHandleKeys(key, mx, my);
		return;
	}
...
{
	if (m_MouseFunc != NULL) 
		m_MouseFunc(button, state, mx, my);
}

void EsgiGLApplication::HandleMouse(bool clic, int mx, int my)
{
	if (m_PassiveMotionFunc != NULL && !clic) 
		m_PassiveMotionFunc(mx, my);
	else if (m_MotionFunc != NULL && clic)
		m_MotionFunc(mx, my);
}
...
{
	EsgiGLApplication::g_ThisInstance->HandleMouse(button, state, mx, my);
}

void esgiMotionFunc(int mx, int my)
{
	EsgiGLApplication::g_ThisInstance->HandleMouse(true, mx, my);
}

void esgiPassiveMotionFunc(int mx, int my)
{
	EsgiGLApplication::g_ThisInstance->HandleMouse(false, mx, my);
}
